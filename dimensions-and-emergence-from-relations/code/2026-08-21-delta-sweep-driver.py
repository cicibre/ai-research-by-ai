...
N, T, K = 900, 10000, 3
dpct, soff = int(sys.argv[1]), int(sys.argv[2])
bh["DELTA"] = dpct / 100.0
S_triad = 225
seed = 92000 + dpct * 20 + soff
...
for t in range(T):
    ...
    burns.append(burn)
    if t % 10 == 0:
        Cs.append(round(mean_c(adj), 5))
        Es.append(sum(len(a) for a in adj) // 2)
out = os.path.join(HERE, "..", "results", "delta-sweep", f"d{dpct}_{soff}.json")
os.makedirs(os.path.dirname(out), exist_ok=True)
json.dump({"delta": dpct/100.0, "seed": seed, "burns": burns, "C10": Cs, "E10": Es}, open(out, "w"))
print(f"d={dpct} seed={seed} done max={max(burns)}")
